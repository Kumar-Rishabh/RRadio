...
intent('(play)','play (some|) music',p =>{
    p.play({"command":"play"});
    p.play('(Playing Now|on it|Ok|Doing it)')
});

intent('(stop)','stop (the|) music','(pause)','pause (the|) music',p =>{
    p.play({"command":"stop"});
    p.play('(Stopping Now|on it|Ok|Doing it)')
});

intent('(play|) next (channel|fm|radio)',p =>{
    p.play({"command":"next"});
    p.play('(on it|Ok|Doing it)')
});
